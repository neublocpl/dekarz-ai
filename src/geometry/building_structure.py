...
def connect_main_lines(
    ...
    for cyc in cycles:
        ...
        if intervals_for_shape:
            corrected_intervals = connect_cycle(intervals_for_shape, scale)
...
def find_all_cycle_containing_interval(
    ...
    for cyc in cycles:
        ...
        if intervals_in_cycle:
            corrected_intervals = connect_cycle(intervals_in_cycle, scale)
            if corrected_intervals is None:
                continue
            results.append(
                Shape(kind="room", intervals=corrected_intervals, polygon=None, is_closed=True)
...
def connect_main_lines_apporach_c(
    ...
    for a in tqdm(main_intervals):
        cycles = find_all_cycle_containing_interval(a, intervals=(all_intervals or []) + main_intervals, pair_paths=pair_paths, scale=scale)
    ...
    for shp in all_shapes:
        for seg in shp.intervals:
            mains_in_cycles.add(_id(seg))

    for a in main_intervals:
        if _id(a) not in mains_in_cycles:
