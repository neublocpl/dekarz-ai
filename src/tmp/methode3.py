...
from fourier import finalne_przetwarzanie_z_detekcja_linii
...
def convert_to_image(_file_path, dpi=300):
    cleared_image = finalne_przetwarzanie_z_detekcja_linii(_file_path)
    try:
        ...
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        return gray, img, cleared_image #cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 21,10)
...
#     print(gray_img.dtype, original_img.dtype, binary_img.dtype)
#     gray_img , original_img, binary_img =binary_img, cv2.cvtColor(binary_img, cv2.COLOR_GRAY2RGB), binary_img
...
def analyze_plan_final(file_path, out_path="out_final_dashed.jpg"):
    ...
    # # Work only with binary image for line detection
    # gray_img, original_img, binary_img = (
    #     binary_img,
    #     cv2.cvtColor(binary_img, cv2.COLOR_GRAY2RGB),
    #     binary_img,
    # )
    ...
    lsd = cv2.createLineSegmentDetector(cv2.LSD_REFINE_STD)
    lines_lsd, _, _, _ = lsd.detect(binary_img)
    ...
    if lines_lsd is not None:
        ...
        for seg in lines_lsd:
            ...
            # --- Classify line type ---
            # Check if yellow
            mask_line = np.zeros(binary_img.shape, dtype=np.uint8)
            cv2.line(
                mask_line,
    ...
    # 2. Detect dashed lines among thin candidates
    dashed_groups, remaining_thin_lines = group_lines_into_dashed(
        lines_by_type["thin"]
    )
    final_lines_to_draw.extend(dashed_groups)

    # 3. Merge remaining solid lines
    final_lines_to_draw.extend(merge_lines(remaining_thin_lines))
    final_lines_to_draw.extend(merge_lines(lines_by_type["normal"]))
    final_lines_to_draw.extend(merge_lines(lines_by_type["thick"]))
    ...
    cv2.imwrite(out_path, output_img)
    print(
        f"Analysis complete. Drew {len(final_lines_to_draw)} final lines to {out_path}"
    )
    return {"count": len(final_lines_to_draw)}
